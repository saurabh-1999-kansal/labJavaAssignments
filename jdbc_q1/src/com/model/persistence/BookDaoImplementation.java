...
public class BookDaoImplementation implements BookDao {
	...
	public void addBook(Book newBook) {
		...
		try {
			PreparedStatement pstmt = connection
					.prepareStatement("insert into book(id,isbn,title,author,price) values(?,?,?,?,?");
	...
	public List<Book> getAll() {
		// we will read all the records from db and put into a arrayList and return to
		// the user
		...
		try {
			...
			while (rs.next()) {
				tempBook = new Book(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getDouble(5));
				books.add(tempBook);
			}
